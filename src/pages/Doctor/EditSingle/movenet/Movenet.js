...
import styles from '../metapipe/MetaPipe.module.css';
...
const Movenet = () => {
    ...
    const { pointMap, point, setPointMap, pointMapMv, setPointMapMv } = useContext(GlobalStorage);
    ...
    const deleteMap = (i) => {
        ...
        let copyMap = [...pointMapMv.front];

        copyMap.splice(i, 1);
        setPointMapMv({ ...pointMapMv, front: copyMap })
    }
    ...
    const handlePosition = (action) => {
        ...
        if (action === 'xl') {
            const value = pointMapMv.front?.map((obj, i) => {
                if (i === ind) {
            ...
            console.log(x);
            setPointMapMv({ ...pointMapMv.front, front: value })
        }
        if (action === 'xr') {
            const value = pointMapMv.front?.map((obj, i) => {
                if (i === ind) {
    ...
    const addPoint = (e) => {
        ...
        const elem = [x, y]
        console.log(elem);
        console.log(pointMapMv);
        // spush(elem)
        const copyEle = [...pointMapMv.front]
        copyEle.push(elem)
        console.log(copyEle);
        setPointMapMv({ ...pointMapMv.front, front: copyEle })
